...
export class DoctorWorkingHoursController {
  ...
  async addWorkingHours(req, res) {
    try {
      ...
      const doctor = await this.workingHoursService.addDoctorWorkingHours(
        id,
        day,
        timeSlot
      );
  ...
  async removeDoctorWorkingHours(req, res) {
    try {
      const { id } = req.params;
      const { day, timeSlot } = req.body;
      const doctor = await this.workingHoursService.removeDoctorWorkingHours(
        id,
        day,
        timeSlot
      );
  ...
  async listDoctorWorkingHours(req, res) {
    try {
      const { id } = req.params;
      const workingHours = await this.workingHoursService.listDoctorWorkingHours(id);
